...
import static custis.easyabac.core.pdp.balana.BalanaModelTransformer.*;
...
public class BalanaPolicyBuilder {
    ...
    static {
        ...
        predefinedAttributes.put(AttributesConstants.ENV_TIME,
                new AttributeDesignator(URI.create(findXacmlName(DataType.TIME)),
                        URI.create(CurrentEnvModule.ENVIRONMENT_CURRENT_TIME),
                        false,
                        URI.create(findXacmlName(Category.ENV))));

        AttributeDesignator currentDate = new AttributeDesignator(URI.create(findXacmlName(DataType.DATE)),
        ...
        AttributeDesignator currentDateTime = new AttributeDesignator(URI.create(findXacmlName(DataType.DATE_TIME)),
                ...
                .map(ra -> new ObligationExpression(Result.DECISION_PERMIT,
                        singletonList(
                                new AttributeAssignmentExpression(URI.create(ra.getId()),
                                        URI.create(findXacmlName(ra.getCategory())),
                                        createAttributeDesignator(ra, false), null)),
                        URI.create(ra.getId()))
                ).collect(Collectors.toSet());
    }

    private List<Rule> buildBalanaRules(List<custis.easyabac.model.Rule> rules, String policyId) {
        return rules.stream()
                .map(r -> buildBalanaRule(r, policyId))
                .collect(toList());
    }

    private Rule buildBalanaRule(custis.easyabac.model.Rule rule, String policyId) {
    ...
    private Condition buildRuleCondition(List<custis.easyabac.model.Condition> conditions,
                                         Operation operation) {
    ...
    private Apply buildRuleConditionApply(custis.easyabac.model.Condition condition) {
        ...
        custis.easyabac.model.Function cf = condition.getFunction();
    ...
    private AttributeValue createAttributeValue(String value, DataType dataType, String conditionId) {
        try {
            return StandardAttributeFactory.getFactory().createValue(
                    URI.create(findXacmlName(dataType)),
    ...
    private Evaluatable createAttributeDesignator(Attribute attribute, boolean selectAsOneValueFromBag) {
        final AttributeDesignator designator = predefinedAttributes.getOrDefault(attribute.getId(),
                new AttributeDesignator(
                        URI.create(findXacmlName(attribute.getType())),
                        URI.create(attribute.getXacmlName()),
                        true,
                        URI.create(findXacmlName(attribute.getCategory()))));
    ...
    private Target buildBalanaTarget(custis.easyabac.model.Target target) {
    ...
    private TargetMatch buildTargetMatch(TargetCondition targetCondition) {
        ...
        final custis.easyabac.model.Function conditionFunction = targetCondition.getFunction();
